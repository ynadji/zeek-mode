...
;; keywords
(defconst zeek-font-lock-keywords-1
  (list
...
(defconst zeek-font-lock-keywords-2
  (append zeek-font-lock-keywords-1
          (list
...
(defvar zeek-font-lock-keywords zeek-font-lock-keywords-2
  "Default highlighting expressions for Zeek mode")

;; indenting
(defun zeek-indent-line ()
  "Indent current line as Zeek code"
  (interactive)
  (beginning-of-line)
  (if (bobp)
      (progn
...
(defun zeek-mode ()
  "Major mode for eding Zeek scripting files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table zeek-mode-syntax-table)
  (use-local-map zeek-mode-map)
  (set (make-local-variable 'font-lock-defaults) '(zeek-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'zeek-indent-line)
  (setq major-mode 'zeek-mode)
  (setq mode-name "Zeek")
  (run-hooks 'zeek-mode-hook))

(defun zeek-event-lookup ()
  "Retrieves the documentation for the event at point.

Requires that the zeek-event-bif be set with a valid path and filename."
  (interactive)
  (let ( (zeek-event-name (thing-at-point 'symbol))
         (start-pos)
         (end-pos)
         (zeek-event-doc)
         (zeek-event-buffer))
    (message "Looking for %s in %s" zeek-event-name zeek-event-bif)
    (if (file-exists-p zeek-event-bif)
        (progn
          (message "Found valid event.bif file.")
          (setq zeek-event-buffer (find-file-noselect zeek-event-bif))
          (save-excursion
            ;; switch to a buffer with the event.bif.zeek file
            (set-buffer zeek-event-buffer)
            (beginning-of-buffer)
            ;; search for the string
            (if (search-forward (format
                                 "global %s: event"
                                 zeek-event-name (beginning-of-buffer))
                                nil 1)
                (progn
                  (end-of-line)
                  (setq end-pos (point))
                  ...
                  (setq zeek-event-doc (buffer-substring start-pos end-pos))
                  (with-output-to-temp-buffer "zeek-event"
                    (princ (format "%s" zeek-event-doc)))
                  (save-window-excursion
                    (save-excursion
                      (switch-to-buffer "zeek-event")
                      (setq buffer-read-only nil)
                      (zeek-mode)))
                  (kill-buffer zeek-event-buffer))
              (message "unable to find the event specified"))))
      (message "Did not find valid event.bif file."))))

(defun zeek-event-query (query)
...
Opens a new buffer with all global events that match the query"
  ...
  (let ((start-pos)
        (end-pos)
        (zeek-query-buffer "zeek-queries")
        (zeek-query-results '()))
    (message "Looking for \"%s\" in %s" query zeek-event-bif)
    (if (file-exists-p zeek-event-bif)
        (progn
          (message "Found valid event.bif file.")
          (setq zeek-event-buffer (find-file-noselect zeek-event-bif))
          (save-excursion
            ...
            (while (re-search-forward (format "global .*%s.*: event" query) nil t)
              (progn
                ...
                (end-of-line)
                (setq end-pos (point))
                (add-to-list 'zeek-query-results (buffer-substring start-pos end-pos))
                (with-output-to-temp-buffer "zeek-event-list"
                  (mapc
                   (lambda (x)
                     (princ (format "%s\n" x)))
                   zeek-query-results))))
            (save-window-excursion
              (save-excursion
                (switch-to-buffer "zeek-event-list")
                (setq buffer-read-only nil)
                (zeek-mode)
...
(defun zeek-insert-event()
  "Insert the built-in function at point into the kill ring."
  (interactive)
  (let ((zeek-event-builtin (buffer-substring
    ...
    (if (looking-at "^global.*event(.*).*;")
      ...
      (message "Not a zeek built in event handler;"))))

(defun zeek-run(tracefile sigfile)
  "Will run the entire buffer through zeek.

Will ask for a tracefile(based on zeek-tracefiles) and a signature file"
  (interactive "sTracefile: \nsSignature file: ")
  (shell-command (format "zeek %s %s %s"
                         (if (equal tracefile "")
                             (concat " ")
                           (concat " -r " zeek-tracefiles "/" tracefile))
                         (if (equal sigfile "")
                             (concat " ")
                           (concat " -s " sigfile))
...
(defun zeek-run-region(tracefile sigfile)
  "Will run the region through zeek
